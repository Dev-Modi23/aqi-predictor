...
# 🔥 EDIT THIS LIST TO MATCH YOUR TRAINING CODE EXACTLY 🔥
# Example below assumes you trained with "PM2.5" (dot) not "PM2_5" (underscore)
FEATURE_COLUMNS = [
    "PM2.5",
    "PM10",
    "NO2",
...
# ================= YOUR ML MODEL PREDICTION (MODEL-ONLY) =================
def predict_aqi(city):
    pm25, pm10, no2 = city_pollution.get(city, [120, 200, 100])

    # Build feature row using EXACT training names
    raw = {
        "PM2.5": pm25,
        "PM10": pm10,
        "NO2": no2,
    ...
    # Ensure DataFrame has the correct column order
    features = pd.DataFrame([[raw[col] for col in FEATURE_COLUMNS]], columns=FEATURE_COLUMNS)
...
def get_city_sources(city_name, current_aqi):
    industrial_cities = [
        "Kanpur", "Ghaziabad", "Ludhiana", "Dhanbad", "Faridabad", "Surat",
        "Panipat", "Durgapur", "Jabalpur", "Solapur", "Kolhapur", "Salem",
        "Warangal", "Asansol", "Bilaspur"
    ]
    vehicle_cities = [
        "Delhi", "Mumbai", "Pune", "Bangalore", "Hyderabad", "Chennai",
        "Thane", "Nashik", "Noida", "Gurugram"
    ]
    construction_cities = [
        "Noida", "Gurugram", "Ahmedabad", "Indore", "Nagpur", "Gwalior",
        "Coimbatore"
    ]

    if city_name in industrial_cities:
        sources = {
            "Factories 🏭": 40, "Vehicles 🚗": 25, "Road Dust 🌫️": 20,
            "Construction 🏗️": 10, "Household 👨‍👩‍👧": 5
        }
    elif city_name in vehicle_cities:
        sources = {
            "Vehicles 🚗": 45, "Factories 🏭": 25, "Road Dust 🌫️": 15,
            "Construction 🏗️": 10, "Household 👨‍👩‍👧": 5
        }
    elif city_name in construction_cities:
        sources = {
            "Construction 🏗️": 35, "Vehicles 🚗": 30, "Factories 🏭": 20,
            "Road Dust 🌫️": 10, "Household 👨‍👩‍👧": 5
        }
    else:
        sources = {
...
st.markdown(
    "<center>🚀 100% ML Model Powered - 60+ Cities Coverage</center>",
    unsafe_allow_html=True
)

# ================= 60 CITIES LIST (DISPLAY) =================
cities_display = [
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["🔮 AI Forecast", "🏭 Source Detection", "🗺️ Live Map", "🚨 Alerts", "🫁 Health Risk"]
)
...
with tab2:
    ...
    with col2:
        ...
        fig_sources.update_traces(textposition='inside', textinfo='percent+label', textfont_size=14)
        fig_sources.update_layout(height=450, title=f"ML Analysis (AQI: {current_aqi})")
